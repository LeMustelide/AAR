...
public class Exemple4Controller {
    ...
    @PostMapping("search")
    public String search(@RequestParam("search") String search, @SessionAttribute("courant") String courant, Model model){
        model.addAttribute("result",facade.findByText(search));
        model.addAttribute("username",courant);
        addMessageToModel(model,courant);
        return "welcome";
    }
